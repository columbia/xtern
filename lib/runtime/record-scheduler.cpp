...
int RRScheduler::fireTimeouts() {
  ...
  for (cur = waitq.begin(); cur != waitq.end();) {
    ...
    if (waits[tid].timeout < turnCount) {
      ...
      waits[tid].reset(ETIMEDOUT); //errno
      waitq.erase(prv);
      runq.push_back(tid);
      ++timedout;
    }
...
bool RRScheduler::idleThreadTryPutTurn() {
  assert(!runq.empty());
  assert(self() == runq.front());
